...
- **Procedural mining assets** — [`scripts/generate_mining_assets.py`](scripts/generate_mining_assets.py) writes PNG sand/gravel textures plus `mining_pit_bowl.obj` / `.mtl` under `assets/mining_generated/` (ignored by git; re-run to regenerate). For use in Blender or a custom loader; not wired into the stock Duckietown tile renderer.
